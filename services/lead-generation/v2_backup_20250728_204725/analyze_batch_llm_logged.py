#!/usr/bin/env python3
"""
Batch LLM Analysis with Guide's Phase 1 Logging System
Enhanced version of analyze_batch_llm.py with comprehensive logging and provenance tracking
"""
...
class LoggedBatchLLMAnalyzer:
    def __init__(self):
        self.client = OpenAI(api_key=os.getenv('OPENAI_API_KEY'))
        self.batch_size = 10  # Process 10 leads at once
        self.logger = V2LoggingSystem()
        
        # Initialize logging system - PRODUCTION ONLY
...
"""
    ...
    def process_batch(self, leads_batch: List[Dict], batch_number: int) -> List[Dict]:
        """Process a batch of leads with LLM and comprehensive logging"""
        ...
        try:
            ...
            for i, lead in enumerate(leads_batch):
                if i < len(scores):
                    ...
                    # Log individual lead qualification
...
def main():
    """Run logged batch LLM analysis with Guide's comprehensive tracking"""
    ...
    print(f"🎯 LOGGED BATCH LLM ANALYSIS - GUIDE'S PHASE 1 SYSTEM")
    print(f"Total leads to process: {len(leads)}")
    print(f"Batch size: 10 leads per API call")
    print(f"Logging: Comprehensive provenance tracking enabled")
    print("=" * 60)
    
    analyzer = LoggedBatchLLMAnalyzer()
    
    # Start processing run
    run_id = analyzer.logger.start_processing_run(
        run_type="v2_full_pipeline_logged",
    ...
    for i in range(0, len(leads), analyzer.batch_size):
        ...
        print(f"📝 Full provenance logged to database")
    ...
    # Save final results
    output_file = f"test_data/logged_batch_results_{time.strftime('%H%M%S')}.json"
    with open(output_file, 'w') as f:
        json.dump(final_results, f, indent=2)
    
    print(f"\n📊 FINAL RESULTS (WITH GUIDE'S LOGGING):")
    ...
    print(f"Processing Method: Logged Batch (10 leads per call)")
    
    print(f"\n💾 Results saved to: {output_file}")
    print(f"📊 Full provenance tracking enabled in database")
    print("✅ LOGGED ANALYSIS COMPLETE - GUIDE'S PHASE 1 SUCCESS!")
